...
import { EmotionRecord, EmotionAnalysis, EmotionType, AdviceRecommendation } from '@/types/emotion'
...
export async function analyzeEmotion(emotionRecord: EmotionRecord): Promise<EmotionAnalysis> {
  try {
    const situationText = `
...
상황 유형: ${emotionRecord.situation.situation_type}
    ...
    const prompt = EMOTION_ANALYSIS_PROMPT
      .replace('{situation}', situationText)
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: '당신은 전문적인 감정 분석 및 심리 상담 AI입니다. 정확하고 도움이 되는 분석과 조언을 제공하세요.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const analysis: EmotionAnalysis = {
      ...
      advice_recommendations: (analysisData.advice_recommendations || []).map((rec: any, index: number) => ({
